...
2. harvestapi/linkedin-profile-posts ($0.002 per post): tracked-creator sweep. Per cycle: at most 10 creators, round-robin through sources/creators.json using rotation_cursor, maxPosts 3 each, postedLimit week, includeReposts false. Input note, observed 2026-08-03 21:00: this actor takes the same postedLimit vocabulary as post-search (any, 1h, 24h, week, month, 3months, 6months, year). The value "past-week" written here until tonight is invalid and returns invalid-input, which costs nothing but burns a step. Input note, observed 2026-08-03: the profile list key is profileUrls. Sending profiles instead returns an empty dataset with status SUCCEEDED and no charge, which reads exactly like a quiet week and is not one; two cycles of "creators returned nothing" were this bug. If a sweep comes back empty, re-read the INPUT record of a known-good run before believing the silence.
...
- Legal tech press by fetch, headline scan: lawsitesblog.com, artificiallawyer.com, law.com/legaltechnews, complexdiscovery.com. Cost note, observed 2026-08-03 21:00: the lawsitesblog.com homepage redirects to lawnext.com and ships its entire product-category taxonomy (well over a thousand list entries) in the fetched text, which is a large share of a cycle's reading for about a dozen headlines. Prefer https://www.lawnext.com/feed or a category page.
- arXiv access note, observed 2026-08-03 21:00: export.arxiv.org no longer fails with DNS resolution, it answers HTTP 429 with a 14-byte body from this sandbox, so the API path is rate-limited or blocked rather than unreachable. The paper pages themselves read fine through the built-in web_fetch (verified on arxiv.org/abs/2604.05485 this cycle). Practical rule until the retro settles it: reach arXiv through web_search and web_fetch on abstract pages, and stop spending steps on the API.
...
- Engagement field path, observed 2026-08-04: harvestapi post results carry counts at engagement.likes, engagement.comments, engagement.shares plus an engagement.reactions breakdown by type. There is no top-level numLikes, reactionsCount or likesCount, and the top-level reactions and reactionIds arrays come back empty even on posts with dozens of reactions. Reading the top-level keys returns None for every post, which looks exactly like a sample with no engagement and would silently flatten a saturation measurement to zero. Parse engagement.likes.
- Legal press cost note, observed 2026-08-04: https://www.lawnext.com/feed returns the full archive as one document, months of items with every image srcset inline, which is a large share of a cycle's reading budget for a headline scan. Its lastBuildDate on this read was 30 July, four days stale, and it produced nothing this lane did not already have. Practical rule: read the feed at most every few cycles, check lastBuildDate first and stop if it predates the last scan, and prefer artificiallawyer.com or a Tavily legal day sweep for same-day legal news.

- URL note, observed 2026-08-04: a Tavily extract 404 reads exactly like a dead source and usually is not one. The Charlotin hallucination database was requested this cycle at /hallucination-cases/, typed from memory, and returned "404 page not found" on three variants before a search showed the live path is /hallucinations/, which is the URL already stored in the queue memo. Third instance of the same failure class after the profiles/profileUrls key and the two wrong creator vanity URLs: read the URL out of state or sources, never out of memory, and treat a 404 on a source that worked yesterday as a suspected typo before reporting it dead.
