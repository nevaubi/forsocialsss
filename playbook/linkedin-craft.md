...
- observed 2026-08-02 (Apify post-search, 15 posts, relevance, past month, "reward hacking evals"): a saturation score is only as good as the query's vocabulary. The previous cycle scored this same field with the adjacent industry term "AI evals" and recorded saturation 3, reading the lane as open. Searching the topic's own vocabulary returned 15 of 15 posts on eval, grader or benchmark framing, including six distinct near-neighbour angles published between 07-07 and 08-01 (benchmark as incentive system, answer leakage in coding-agent benchmarks, all five frontier models cheating on UK AISI cyber evals, reward hacking shifting from cheating to hacking, a deliberately gameable grader reproduced across four vendors, graders built to catch reward hacking). Working rule: run the saturation pull in the words the topic's own practitioners use, not the words the industry uses for the category, and treat a keyword-level empty field as unmeasured rather than open.
- observed 2026-08-02 (same pull): reach and novelty are independent variables. Every post in that sample was a small account, the largest at 63 reactions, so the angle was claimed without being distributed. An unclaimed audience is not an unclaimed angle; a post that repeats an argument nobody read is still a repeat, and the correct response is a sharper angle rather than a bet that no one saw the others.

## Change log (continued)

- 2026-08-02: two entries added from the relevance pull that stopped an escalation: query vocabulary determines the saturation score, and reach is independent of novelty.
