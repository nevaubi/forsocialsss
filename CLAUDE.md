...
- Slack: use the Slack Web API with SLACK_BOT_TOKEN. Resolve the DM once per cycle with conversations.open for user U0BM0RF8AHM and deliver with chat.postMessage. Directive reads use an explicit cursor: read messages with conversations.history strictly newer than last_read_ts in state/slack-cursor.json, process them, then write the newest ts you saw back to that file and commit it with the cycle state. Never advance the cursor past a directive you failed to process; leave it so the next cycle retries. If SLACK_BOT_TOKEN is absent, write each would-be message to outbox/<UTC timestamp>-<type>.md, commit them, and note the degraded mode in the run log; drafts still expire per state/SCHEMA.md.
...
- A second agent shares this DM: the assistant (Kimi K3, assistant/assistant.py), an agentic tool loop with read, exact-match edit, write, verification shell, diff review, web research, and direct commit-and-push to main (assistant: commits, author assistant@forsocialsss), plus revert on request, PRs only when Firas explicitly asks for one, and document generation (PDF reports, decks, infographics per assistant-skills/ recipes) delivered as Slack uploads. Division of labor: you own the content pipeline and the reply keywords listed above; the assistant handles everything conversational and configurational, and marks what it has answered with a robot_face reaction. Ignore messages carrying that reaction unless they are your keywords. Treat assistant commits on main like any other upstream change: pull them at boot and follow the current state of the files.
