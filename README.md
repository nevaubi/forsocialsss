...
A second agent runs alongside the heartbeat: Kimi K3 on the Fireworks serverless endpoint, polling the same Slack DM every 5 minutes (assistant/assistant.py, Assistant Chat workflow). No command syntax: a router classifies each message. Questions get grounded answers from the repo, run log, and live deployment status. Plain-language instructions ("raise the news weight to 25", "tighten the draft in the queue") are executed immediately: Kimi rewrites the target files completely, the assistant commits straight to main, and the reply includes the commit link. "Revert that" undoes the last assistant commit. Asking for a pull request routes the change through the Assistant Apply workflow for review instead. Structural guards: path traversal blocked, the hard rules section of CLAUDE.md cannot be silently removed, and any edit gutting most of a file is held until the word confirm appears. The heartbeat pulls main every cycle, so changes take effect on its next fire. Handled messages get a robot_face reaction; heartbeat keywords (approve, edit:, kill:, hold, status, posted, retro now) are left for the heartbeat.
